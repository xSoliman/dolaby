...
function WearLogForm({ onCancel, onDone }: { onCancel: () => void; onDone: () => void }) {
  ...
  const { notify } = useToast();
  const readyOutfits = outfits.filter((outfit) => !outfit.isDraft && !outfit.isArchived);
