...
export default function OutfitsPage() {
  const { outfits, items, addOutfit, deleteOutfit, updateOutfit, setOutfitArchived } = useWardrobe();
  ...
  const base = useMemo(() => outfits.filter((outfit) => showArchived ? outfit.isArchived : !outfit.isArchived), [outfits, showArchived]);
  const archivedCount = outfits.filter((outfit) => outfit.isArchived).length;
  const filtered = useMemo(() => base.filter((outfit) => {
  ...
  const toggleArchive = async (outfit: Outfit) => {
    try {
      await setOutfitArchived(outfit.id, !outfit.isArchived);
      setMenu(null);
      notify(outfit.isArchived ? "Outfit restored." : "Outfit archived.");
    } catch (error) {
      notify(errorMessage(error, "Could not update this outfit."), "error");
    }
  };
    ...
    <div className="page-stack outfits-page">
      ...
      <div className="outfit-filters">
        <div className="closet-tabs compact-tabs">
          <button className={status === "all" ? "active" : ""} onClick={() => setStatus("all")}>All <span>{base.length}</span></button>
          <button className={status === "ready" ? "active" : ""} onClick={() => setStatus("ready")}>Ready <span>{base.filter((outfit) => !outfit.isDraft).length}</span></button>
          <button className={status === "draft" ? "active" : ""} onClick={() => setStatus("draft")}>Drafts <span>{base.filter((outfit) => outfit.isDraft).length}</span></button>
        </div>
      ...
      {archivedCount ? <div className="collection-meta"><p><strong>{archivedCount}</strong> archived</p><div><button className="text-link" onClick={() => setShowArchived((value) => !value)}>{showArchived ? "← Back to active outfits" : "Show archived"}</button></div></div> : null}
      ...
      {filtered.length ? (
        <div className="outfits-grid">
            ...
            return (
              <article className="outfit-card" key={outfit.id}>
                ...
                <div className="outfit-card-copy">
                  ...
                  <div className="context-menu-wrap"><button onClick={() => setMenu(menu === outfit.id ? null : outfit.id)} aria-label="Outfit options"><MoreHorizontal size={19} /></button>{menu === outfit.id ? <div className="context-menu"><Link href={`/outfits/new?edit=${outfit.id}`}><Pencil size={14} /> Edit</Link>{outfit.isDraft ? <button onClick={() => void publish(outfit)}><CheckCircle2 size={14} /> Mark ready</button> : null}<button onClick={() => void duplicate(outfit)}><Copy size={14} /> Duplicate</button><button onClick={() => void toggleArchive(outfit)}><Archive size={14} /> {outfit.isArchived ? "Unarchive" : "Archive"}</button><button className="danger" onClick={() => { setDeleteTarget(outfit); setMenu(null); }}><Trash2 size={14} /> Remove</button></div> : null}</div>
                </div>
          ...
          {showArchived ? null : <Link href="/outfits/new" className="add-outfit-card"><span><Plus size={24} /></span><strong>Build a new outfit</strong><small>Choose a few pieces that work together</small></Link>}
        </div>
      ) : (
        <EmptyState icon={Sparkles} title={base.length ? "No outfits match these filters" : "Your first easy morning starts here"} copy={base.length ? "Try another occasion or include both ready looks and drafts." : "Combine pieces you already enjoy wearing. Name it now or keep it as a draft."} action={!base.length ? <Link href="/outfits/new" className="button button-primary button-md"><Plus size={16} /> Build an outfit</Link> : <Button variant="secondary" onClick={() => { setStatus("all"); setOccasion("all"); }}>Clear filters</Button>} />
      )}
